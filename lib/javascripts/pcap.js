...
function renderPcaps (state, reRender) {
  ...
  var pcaps = state.pcaps.filter(function (pcap) {
    ...
    if (flag && state.filterPort) {
      var sport = pcap.result.sport || pcap.sport;
      var dport = pcap.result.dport || pcap.dport;
      flag = (state.filterPort === sport || state.filterPort === dport);
    }
  ...
  for (var i = (page - 1) * perPage, len = Math.min(pcaps.length, page * perPage); i < len; ++i) {
    ...
    var pcap = pcaps[i];
    var detail = Object.assign({}, pcap, pcap.result);
