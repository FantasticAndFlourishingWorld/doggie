...
$(document).ready(function () {
  ...
  $('.settings-blacklist-submit').click(function () {
    ...
    var urlRegExp = new RegExp('^(?:((http|https):\\/\\/)?[\\w\\-_]+(\\.[\\w\\-_]+)+([\\w\-\\.,@?^=%&amp;:/~\\+#]*[\\w\\-\\@?^=%&amp;/~\\+#])?)$');
    ...
    if (!url) {
      ...
      var urlShell = new PythonShell('limit_http.py', {
        mode: "json",
        scriptPath: __dirname + '/../py',
        args: ['add', url]
      });

      urlShell.on('message', function (res) {
        if (!res.success) {
          urlShell.end(function () {});
      ...
      urlShell.on('error', function (err) {
        console.error('wrong');
        urlShell.end(function () {});
      });
...
function removeUrl () {
  var blacklist = utils.readSettings('blacklist');
  var index = $('.settings-blacklist-delete').index($(this)[0]);
  var removedUrl = blacklist[index];

  var urlShell = new PythonShell('limit_http.py', {
    mode: "json",
    scriptPath: __dirname + '/../py',
    args: ['delete', removedUrl]
  });
